...
const createListener = <Type extends keyof WindowEventMap>(type: Type, fn: (event: WindowEventMap[Type]) => void) => {
...
export const Modal = (iprops: ModalProps) => {
  ...
  const Content = () => {
    onMount(() => {
      ...
      createListener("keydown", (event) => event.key === "Escape" && modal().close());
    });
    onCleanup(() => {
      props.onClose?.();
      Modals.update();
